...
add_action( 'admin_enqueue_scripts', 'pte_admin_pointers_header' );

	function pte_admin_pointers_header() {
		if ( pte_admin_pointers_check() ) {
			add_action( 'admin_print_footer_scripts', 'pte_admin_pointers_footer' );
	...
	function pte_admin_pointers_check() {
		$admin_pointers = pte_admin_pointers();
	...
	function pte_admin_pointers_footer() {
		$admin_pointers = pte_admin_pointers();
		?>
	...
	function pte_admin_pointers() {
		$dismissed = explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_wp_pointers', true ) );
		$version = '1_0'; // replace all periods in 1.0 with an underscore
		$prefix = 'pte_admin_pointers' . $version . '_';

		$new_pointer_content = '<h3>' . __( 'Setup Press This Extended' ) . '</h3>';
		$new_pointer_content .= '<p>' . __( 'Modify the default settings to Press This in the Writing section.' ) . '</p>';
